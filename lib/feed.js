...
var filter = [
  'addAttachmentToCard', 'addChecklistToCard', 'commentCard', 'createCard', 'createCard',
  'deleteCard', 'moveCardToBoard', 'moveListToBoard', 'updateBoard', 'updateCard',
  'updateCheckItemStateOnCard', 'updateChecklist', 'updateList', 'updateMember', 'updateOrganization'
]
...
var get = () => new Promise((resolve, reject) =>
  ...
  .then(([res, boards]) =>
    Promise.all(boards
      ...
      .map((board) => trello.get('boards/' + board.id + '/actions')
        .qs({since: db[env].boards[board.id], filter}).request()))
    .then(resolve)
    .catch(reject)
  )
  .catch(reject)
